...
| `pydantic-ai-slim[openai]` | The agentic framework behind the **authoring agent** (Phase 8 slice 3) — native tool-calling over OpenRouter's OpenAI-compatible endpoint (`qwen/qwen3.6-27b`), tools as typed Python functions whose args are validated *before* the tool body runs, and — decisively for the offline gate — `TestModel`/`FunctionModel` to drive the whole agent deterministically with **no network**. A deliberate departure from the core policy: the authoring agent is a **separate stack** from the engine's slim `LLMProvider` (design decision 2026-07-22), so it may carry a heavier tree (`openai`, `pydantic`, `opentelemetry-api`, `tiktoken`, `pydantic-graph`). The same rule keeps it honest: **`loom/` never imports it** — the agent lives in `authoring/`, wraps the pure `loom/` capabilities as tools, and the shadow-validate-apply safety gate stays in tested `loom/` code, never in the framework. The `-slim` distribution + the `[openai]` extra keeps the tree to the OpenAI-compatible provider only (not every model backend). Install only when authoring: `pip install -e ".[authoring]"`. |
